...
| | |
|---|---|
| `origin` | `https://github.com/laweirdo/bulan-qt.git` — Bulan, where your work goes |
...
The repository was renamed from `moonlight-qt` to `bulan-qt` on 9 August 2026.
GitHub redirects the old URL, so a clone made before then keeps working; run
`git remote set-url origin https://github.com/laweirdo/bulan-qt.git` to stop
