...
win32 {
    ...
    # What Windows shows in the file's Properties pane and in Task Manager.
    # Presentation only -- none of these is an upgrade or install identity, so
    # they carry Bulan. QMAKE_TARGET_COMPANY stays Moonlight's: it matches the
    # registry and install paths in wix/, which are upgrade identity and must
    # not move.
    QMAKE_TARGET_COMPANY = Moonlight Game Streaming Project
    QMAKE_TARGET_DESCRIPTION = Bulan
    QMAKE_TARGET_PRODUCT = Bulan
