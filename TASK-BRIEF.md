...
**Status, 3 August 2026: every implementation stage is built.** Stages 1 and 2
were accepted and merged into `bulan`; stages 3, 4, 5 and the app-icon half of
6 are on `v1-review-build` and **have not been seen by the client**.

This brief stays open. Its remaining work is not implementation: a client
review, a Flatpak build, and the Steam Deck validation that cannot happen off
the target device. `HANDOFF.md` records what was and was not checked.
...
| 4 | Edge states: zero hosts, unreachable host, empty library, couldn't start stream, disconnect confirmation, wake success/timeout/failure | Every state has a controller-safe recovery route and preserves context | **Done** — `a0384d62`. Unreachable host, couldn't start stream, the wake states and the disconnect (quit) confirmation already existed from earlier phases; the zero-hosts and empty-library states were built here |
| 5 | Settings shell and About: spatial D-pad navigation, custom Bulan controls, atmosphere-effect flags, "Built on Moonlight" attribution, grid SELECT → host settings | No stock Qt Quick Controls in a Bulan screen; usable without a mouse | **Done** — `2f91a306` for the shell and About, `a0384d62` for grid SELECT → host settings |
