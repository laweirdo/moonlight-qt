...
| Item | State |
|---|---|
| Branch | **`polish-input-startup`**, three commits ahead of `master`. Not merged, **not pushed** |
...
changed QML files with categories unchanged against `master`; the hint bar
captured on Recent and Library; and a real launch reaching `Steambox`'s actual
library — which also settles real box art at a scaled resolution, previously
untested.

**Passed on hardware.** With a real controller attached, the client walked the
...
**Not established.** The stage 3 live checks and the post-Settings A-button
regression were **not reported as performed** — the validation report lists them
individually. **No Deck ran**, in Desktop or Game Mode; Game Mode is the larger
gap, because Steam Input sits between the hardware and the app there. Two
controllers at once was never driven. Three of the four startup cases — host
offline, UUID gone, no remembered host — were not run, the review station's
stored preferences having been left alone deliberately. macOS and the MSI were
not built.
...
1. **The unconfirmed checks in the validation report** — chiefly A still working
   after Client Settings and after the Resolution dropdown. Cheapest thing
   outstanding, and the checklist calls losing A a stop-the-review failure.
